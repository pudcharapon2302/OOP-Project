...
page_bg_img = '''<style>
.stApp {
    background-image: url("https://images.unsplash.com/photo-1546519638-68e109498ffc?q=80&w=2090&auto=format&fit=crop&ixlib=rb-4.0.3&ixid=M3wxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHx8fA%3D%3D");
...
def main():
    ...
    'โปรแกรมหัดเล่นบาส 🏀',
    ("To be a Kyrie Irving",'การเลี้ยงลูก (Dribbling) 🏀', 'การชู๊ต (Shooting) 🏀', 'การส่งลูก (Passing) 🏀'))

    if option == 'การเลี้ยงลูก (Dribbling) 🏀':

        st.markdown("<h2 style='color: white; text-shadow: 2px 2px black;'>1.การเลี้ยงลูกบาสเกตบอลสูง (High Dribbling)</h2>", unsafe_allow_html=True)
        st.markdown("<p style='color: white;'>การเลี้ยงลูกบาสเกตบอลสูง เป็นการเลี้ยงลูกที่ต้องใช้ความเร็ว ส่วนใหญ่ทักษะการเลี้ยงลูกบาสเกตบอลแบบสูงได้มักจะใช้ในจังหวะที่อยู่ในพื้นที่กว้างมากพอที่จะเคลื่อนที่ไปข้างหน้าได้อย่างรวดเร็ว โดยมือที่เลี้ยงลูกไม่จำเป็นต้องอยู่ด้านบนเท่านั้น แต่สามารถอยู่ด้านหลังเพื่อใช้ความเร็วผลักลูกบาสไปข้างหน้าได้</p>", unsafe_allow_html=True)
        st.markdown("""
            1. <font color='white'>โน้มลำตัวไปข้างหน้าเล็กน้อย มองจุดหมายด้านหน้า ศีรษะตั้งตรง</font>
            2. <font color='white'>กางมือออก บังคับลูกบาสด้วยปลายนิ้วโดยใช้แรงส่งจากข้อมือ</font>
            3. <font color='white'>ทำการผลักลูกบาสลงพื้นให้เด้งขึ้นระดับเอว แต่ไม่เกินระดับอก</font>
            4. <font color='white'>เคลื่อนที่ตามไปพร้อมกับการเลี้ยงลูกบาสเกตบอลไปข้างหน้า</font>
            """, unsafe_allow_html=True)
        url_01 = 'https://youtu.be/eWIiXP4RutQ?si=jD7EIjhp56cG6YXP'
        st.video(url_01)

        st.markdown("<h2 style='color: white; text-shadow: 2px 2px black;'>2. การเลี้ยงลูกบาสเกตบอลต่ำ (Low Dribbling)</h2>", unsafe_allow_html=True)
        st.markdown("<p style='color: white;'>การเลี้ยงลูกบาสเกตบอลต่ำเป็นวิธีเลี้ยงลูกบาสให้อยู่ใกล้ตัว โดยควบคุมลูกบาสไม่ให้ถูกฝ่ายตรงข้ามแย่งไป ซึ่งการเลี้ยงลูกบาสให้อยู่ในระดับต่ำจะเป็นการเลี้ยงลูกเมื่อถูกฝ่ายตรงข้ามป้องกันอย่างใกล้ชิด</p>", unsafe_allow_html=True)
        st.markdown("""
            1. <font color='white'>โน้มตัวไปข้างหน้า กางขาย่อเข่าลงต่ำ</font>
            2. <font color='white'>กดลูกบาสลงสู่พื้น เมื่อลูกบาสเด้งขึ้นมาให้กดข้อมือลงไปใหม่</font>
            3. <font color='white'>หากเลี้ยงลูกบาสด้วยมือขวา ทำการกางแขนซ้ายเพื่อทรงตัวและป้องกันการถูกแย่งจากฝั่งตรงข้าม</font>
            4. <font color='white'>ใช้ปลายนิ้วมือในการเลี้ยงลูกบาสเกตบอลให้ไม่เกินระดับเข่าบังคับไปในทิศทางที่ต้องการ</font>
            """, unsafe_allow_html=True)
        url_02 = 'https://www.youtube.com/watch?v=UnRklE9VON4&pp=ygUPKExvdyBEcmliYmxpbmcp'
        st.video(url_02)

        st.markdown("<h2 style='color: white; text-shadow: 2px 2px black;'>3. การเลี้ยงลูกบาสเกตบอลเปลี่ยนตำแหน่ง (The Change-of-Pace Dribble)</h2>", unsafe_allow_html=True)
        st.markdown("<p style='color: white;'>ทักษะการเลี้ยงลูกบาสเกตบอลเปลี่ยนตำแหน่งเป็นการทำให้ฝั่งตรงข้ามไม่สามารถคาดเดาได้ว่าคุณจะเคลื่อนไหวไปทางไหน เคลื่อนที่เร็วหรือช้า หยุดอยู่กับที่หรือจะส่งลูกบาสไปในทิศทางใด เป็นต้น โดยวิธีการเลี้ยงลูกบาสเกตบอลแบบนี้ขึ้นอยู่ว่าคุณต้องการจะให้ลูกบาสไปในทิศทางไหนด้วยวิธีใด ถือเป็นเรื่องเฉพาะตัวที่ต้องมีการวางแผนกันในทีม</p>", unsafe_allow_html=True)
        url_03 = 'https://www.youtube.com/watch?v=0Wwv5nDpL6k&pp=ygUcKFRoZSBDaGFuZ2Utb2YtUGFjZSBEcmliYmxlKQ%3D%3D'
        st.video(url_03)

        st.markdown("<h2 style='color: white; text-shadow: 2px 2px black;'>4. การเลี้ยงลูกบาสเกตบอลสลับซ้าย-ขวา (The Crossover Dribbling)</h2>", unsafe_allow_html=True)
        st.markdown("<p style='color: white;'>ทักษะการเลี้ยงลูกบาสเกตบอลแบบสลับซ้าย-ขวาเป็นการเลี้ยงลูกโดยหลบหลีกไม่ให้ฝ่ายตรงข้ามเข้ามาปัดลูกบาสได้ง่ายๆ ซึ่งจะทำการย้ายมือข้างที่เลี้ยงลูกบาสสลับไปมา สำหรับวิธีเลี้ยงลูกบาสแบบนี้หากไม่ชำนาญอาจถูกแย่งหรือปัดได้ง่ายจากฝ่ายตรงข้าม ซึ่งการฝึกบ่อยๆ ช่วยให้มีความชำนาญขึ้นได้</p>", unsafe_allow_html=True)
        st.markdown("""
            1. <font color='white'>ย่อตัวลงเล็กน้อย สายตาจ้องไปด้านหน้า</font>
            2. <font color='white'>เลี้ยงลูกบาสด้วยปลายนิ้วข้างที่ถนัด</font>
            3. <font color='white'>เมื่อฝ่ายตรงข้ามเข้ามาใกล้ให้ทำการส่งลูกบาสไปด้านตรงข้ามทันที</font>
            4. <font color='white'>ใช้แรงส่งจากข้อมือไปยังปลายนิ้วมืออีกด้านเพื่อเลี้ยงลูกบาสไว้ ใส่ข้อมูลนี้ลงไปแทน</font>
            """, unsafe_allow_html=True)
        url_04 = 'https://www.youtube.com/watch?v=M3yD-gwjoRE&pp=ygUZKFRoZSBDcm9zc292ZXIgRHJpYmJsaW5nKQ%3D%3D'
        st.video(url_04)

        st.markdown("<h2 style='color: white; text-shadow: 2px 2px black;'>5. การเลี้ยงลูกบาสเกตบอลไขว้หลัง (The Behind-the-Back Dribble)</h2>", unsafe_allow_html=True)
        st.markdown("<p style='color: white;'>การเลี้ยงลูกบาสเกตบอลไขว้หลังนั้นเป็นการเลี้ยงลูกบาสเกตบอล เพื่อหลอกล่อให้ฝ่ายตรงข้ามเสียจังหวะในการเข้ามาแย่งชิงลูกบาสไปและเป็นการทำให้คุณได้เปรียบในการชิงพื้นที่ ที่เปลี่ยนตำแหน่งหรือเคลื่อนไหวไปในทิศทางอื่น</p>", unsafe_allow_html=True)
        st.markdown("""
            1. <font color='white'>โน้มตัวไปข้างหน้า ศีรษะตั้งตรงเพื่อการทรงตัวที่ดี</font>
            2. <font color='white'>ใช้มือออกแรงผลักลูกบาสจากด้านหลังไปด้านหน้าอีกข้างหนึ่ง</font>
            3. <font color='white'>จากนั้นทำการเลี้ยงลูกบาสเกตบอลไขว้สลับกันเพื่อชิงพื้นที่จากฝ่ายตรงข้าม</font>
            """, unsafe_allow_html=True)
        url_05 = 'https://www.youtube.com/watch?v=T49ZW9cyuWs&pp=ygUdKFRoZSBCZWhpbmQtdGhlLUJhY2sgRHJpYmJsZSk%3D'
        st.video(url_05)

        st.markdown("<h2 style='color: white; text-shadow: 2px 2px black;'>6. การเลี้ยงลูกบาสเกตบอลลอดใต้ขา (The Between-the-Legs-Dribble)</h2>", unsafe_allow_html=True)
        st.markdown("<p style='color: white;'>อีกหนึ่งทักษะการเลี้ยงลูกที่ควรฝึก คือ การเลี้ยงลูกบาสเกตบอลลอดใต้ขา เพื่อหลบหลีกการป้องกันในระยะประชิดของฝ่ายตรงข้าม ซึ่งการเลี้ยงลูกบาสเกตบอลแบบนี้จะทำให้สามารถเปลี่ยนทิศทางของฝ่ายตรงข้าม และทำให้ฝั่งตรงข้ามเสียพื้นที่และจังหวะในการเข้ามาแย่งลูกบาสได้ ซึ่งช่วยให้ทีมเรามีโอกาสในการทำแต้มได้ง่ายขึ้น</p>", unsafe_allow_html=True)
        st.markdown("""
            1. <font color='white'>เมื่อฝ่ายตรงข้ามเข้ามาในระยะประชิดให้สายตามองไปยังจุดหมายที่ต้องการเคลื่อนที่</font>
            2. <font color='white'>ใช้ปลายนิ้วมือกดลูกบาสให้ลอดใต้ขาไปอีกฝั่ง</font>
            3. <font color='white'>เคลื่อนย้ายไปยังจุดที่ต้องการพร้อมกับการเลี้ยงลูกบาสเกตบอล ไปด้วย</font>
            """, unsafe_allow_html=True)
        url_06 = 'https://www.youtube.com/watch?v=zIxM7CUFBcw&pp=ygUeKFRoZSBCZXR3ZWVuLXRoZS1MZWdzLURyaWJibGUp'
        st.video(url_06)

        st.markdown("<h2 style='color: white; text-shadow: 2px 2px black;'>7. การหมุนตัวเลี้ยงลูกบาสเกตบอล (The Reverse Dribble)</h2>", unsafe_allow_html=True)
        st.markdown("<p style='color: white;'>ทักษะการเลี้ยงลูกบาสเกตบอลทักษะสุดท้ายที่จะนำมาแนะนำวันนี้เป็นการหมุนตัวเลี้ยงลูกบาสเกตบอล เพื่อเปลี่ยนทิศทางและหลบหลีกฝ่ายตรงข้ามให้เข้ามาปัดหรือแย่งลูกบาสได้ยากมากขึ้น</p>", unsafe_allow_html=True)
        st.markdown("""
            1. <font color='white'>เมื่อฝ่ายตรงข้ามเข้ามาประชิดตัวจะทำการใช้มือผลักลูกบาสส่งออกไปอีกด้านหนึ่ง</font>
            2. <font color='white'>หมุนตัวไปตามด้านนั้นเดียวกันเพื่อรับลูกบาส</font>
            3. <font color='white'>สามารถทำการหมุนตัวเพื่อเคลื่อนที่ได้ในทุกทิศทาง</font>
            """, unsafe_allow_html=True)
        url_07 = 'https://www.youtube.com/watch?v=skK0uGQemLE&pp=ygUWIChUaGUgUmV2ZXJzZSBEcmliYmxlKQ%3D%3D'
        st.video(url_07)

    if option == 'การชู๊ต (Shooting) 🏀':

        st.markdown("<h2 style='color: white; text-shadow: 2px 2px black;'>1.การยืนชู้ต One Hand Set Shot</h2>", unsafe_allow_html=True)
        st.markdown("<p style='color: white;'>รูปแบบการยืนชู้ตแบบ one hand นั้นเหมาะสำหรับการชู้ตลูก 2 แต้มคือผู้เล่นต้องอยู่ไม่ไกลจากห่วงนัก หรือไม่ก็เป็นตอนที่ได้ยิงลูกโทษ โดยการชู้ตแบบนี้ผู้เล่นจะไม่ได้กระโดด เพียงแต่ยืดแขนออกไปโดยใช้แขนข้างใดข้างนึงเป็นตัวนำส่วนอีกมือนึงเป็นตัวประครอง ดังนั้นการชู้ดรูปแบบนี้จะชู้ดในมุมต่ำ เวลาใช้ในสนามก็ควรชู้ดต่อเมื่อไม่มีคู่ต่อสู้ค่อยขัดขวาง</p>", unsafe_allow_html=True)
        st.markdown("""
            1. <font color='white'>เริ่มจากเท้าควรว่างเท้าใดเท้านึงนำเล็กน้อย หรืออาจจะวางเท้าในระดับเดียวกันแต่ห่างออกจากกันก็ได้</font>
            2. <font color='white'>ย่อเข่าลงเล็กน้อยใช้มือข้างที่ถนัดจับอยู่ที่ใต้ลูกบาส ส่วนอีกมือก็ช่วยประคองลูก ยกลูกขึ้นเหนือศีรษะ</font>
            3. <font color='white'>สายตาเล็งไปที่ห่วง หรืออาจจะเล็งไปที่กรอบสี่เหลี่ยมก็ได้</font>
            4. <font color='white'>ชู้ดบาส โดยการส่งแรงจากหัวเข่าขึ้นมา แขนยืนตรงออกไปให้สุด ใช้ข้อมือสะบัดลูกออกไป</font>
            """, unsafe_allow_html=True)
        url_08 = 'https://www.youtube.com/watch?v=Wv66_-GKNlQ&pp=ygURT25lIEhhbmQgU2V0IFNob3Q%3D'
        st.video(url_08)

        st.markdown("<h2 style='color: white; text-shadow: 2px 2px black;'>2.การกระโดดชู้ต Jump Shoot</h2>", unsafe_allow_html=True)
        st.markdown("<p style='color: white;'>การกระโดดชู้ตนั้นเป็นท่าชู้ดที่ผู้เล่นส่งแรงจากการกระโดดขึ้นมาทำให้สามารถชู้ตได้ในระยะที่ไกลเหมาะสำหรับการทำแต้ม 3 คะแนน และยังสามารถใช้ในการชู้ดที่มีผู้เล่นฝ่ายตรงข้ามเข้ามาขัดขวางได้</p>", unsafe_allow_html=True)
        st.markdown("""
            1. <font color='white'>ย่อเข่าลงเพื่อเตรียมกระโดดขึ้น มือทั้งสองขัางจับบอลอยู่ในระดับหน้าอก</font>
            2. <font color='white'>สายตาเล็งไปยังห่วงบาส</font>
            3. <font color='white'>ยกมือขึ้นเหนือศีรษะ มือข้างที่ถนัดอยู่ใต้ลูกส่วนอีกมือช่วยประคองลูก ศอกและแขนตั้งตรงไม่เอียง</font>
            4. <font color='white'>กระโดดขึ้นส่งแรงจากข้อเท้าขึ้นมา แขนยืดตรงใช้ข้อมือสบัดลูกออกไปสิ่งสำคัญสำหรับการชู้ตบาสทั้งสองแบบก็คือการใช้ข้อมือในการสะบัดลูกให้ออกไปในทิศทาง หรือวิถีโค้งที่เราต้องการ</font>
            """, unsafe_allow_html=True)
        url_09 = 'https://www.youtube.com/watch?v=k6eqU7XRA1o&pp=ygUKSnVtcCBTaG9vdA%3D%3D'
        st.video(url_09)

    if option == 'การส่งลูก (Passing) 🏀':

        st.markdown("<h2 style='color: white; text-shadow: 2px 2px black;'>1.การส่งลูกสองมือ</h2>", unsafe_allow_html=True)
        st.markdown("<p style='color: white;'>การส่งลูกสองมือ จะใช้มือทั้งสองส่งลูกออกไป มีวิธีการส่งหลายแบบที่นิยม ได้แก่ การส่งลูกสองมือระดับหน้าอก และการส่งลูกสองมือเหนือศีรษะการส่งลูกสองมือระดับอก ใช้สำหรับการส่งระยะไม่ไกลนัก คือ ระยะทางประมาณ 3-8 เมตร ถือว่าเป็นการส่งที่ใช้มากที่สุด</p>", unsafe_allow_html=True)
        st.markdown("""
            1. <font color='white'>จากท่ายืนถือลูกครอบครองบอล หุบศอก ดึงลูกบอลเข้าหาตัวคล้ายม้วนมือขึ้น แล้วผลักออกไปด้วยมือทั้งสองข้าง</font>
            2. <font color='white'>ขณะส่งลูกออกไปแขนทั้งสองเหยียดตรงตามทิศทางที่ส่งสายตามองเป้าหมาย</font>
            3. <font color='white'>ถ้าต้องการส่งลูกให้แรงขึ้น ให้ก้าวเท้าข้างหนึ่งตามไป โดยอย่าให้เท้าอีกข้างหนึ่งเคลื่อนที่</font>
            4. <font color='white'>ใช้นิ้วมือเป็นตัวบังคับทิศทางลูกให้ตรงตามต้องการ</font>
            """, unsafe_allow_html=True)
        url_10 = 'https://www.youtube.com/watch?v=HtAeT_ubf0o&pp=ygUedHdvIGhhbmQgcGFzcyBiYXNrZXRiYWxsIGNoZXN0'
        st.video(url_10)

        st.markdown("<h2 style='color: white; text-shadow: 2px 2px black;'>2.การส่งลูกมือเดียว </h2>", unsafe_allow_html=True)
        st.markdown("<p style='color: white;'>การส่งลูกมือเดียว คือ การใช้มือเดียวส่งลูกออกไป มีวิธีการส่งหลายวิธีที่นิยมใช้กันมาก ได้แก่ การส่งลูกมือเดียวเหนือไหล่ และการส่งลูกมือเดียวข้างลำตัวการส่งลูกมือเดียวเหนือไหล่ ใช้สำหรับการส่งลูกระยะไกล</p>", unsafe_allow_html=True)
        st.markdown("""
            1. <font color='white'>จากท่ายืนถือครอบครองบอล ยกลูกขึ้นเหนือไหล่ด้วยมือที่ต้องการส่งลูก โดยให้มือที่จะใช้ส่งตั้งข้อมือหันฝ่ามือออก</font>
            2. <font color='white'>ก้าวเท้าด้านตรงข้ามกับมือที่ส่งไปข้างหน้า</font>
            3. <font color='white'>ส่งแรงที่แขนและข้อมือที่ส่งไปตามทิศทางที่ต้องการ</font>
            4. <font color='white'>ใช้นิ้วมือบังคับทิศทางลูก</font>
            """, unsafe_allow_html=True)
        url_11 = 'https://www.youtube.com/watch?v=D-dQBl1es4Y&pp=ygUeb25lIGhhbmQgcGFzcyBiYXNrZXRiYWxsIGNoZXN0'
        st.video(url_11)

        st.markdown("<h2 style='color: white; text-shadow: 2px 2px black;'>3.การส่งลูกกระดอน </h2>", unsafe_allow_html=True)
        st.markdown("<p style='color: white;'>การส่งลูกกระดอน คือ การส่งลูกให้ตกพื้นก่อนถึงตัวผู้รับ ใช้สำหรับการส่งระยะไม่ไกลนัก เหมาะกับผู้เล่นที่ต้องการส่งลูกหลบฝ่ายตรงข้ามที่ตัวสูง มีวิธีการส่งหลายวิธี สามารถส่งได้ทั้งมือเดียวและสองมือ วิธีการส่งคล้ายกับการส่งโดยทั่วไป แต่ต้องส่งลูกให้กระทบพื้นถึงตัวผู้รับเป็นระยะ 2 ใน 3 ส่วนของระยะทางทั้งหมด ในที่นี้จะกล่าวถึงการส่งลูกสองมือกระดอนเท่านั้น</p>", unsafe_allow_html=True)
        st.markdown("""
            1. <font color='white'>ต้องส่งลูกได้ตรง แม่นยำ มีความเร็วและมีความแรงพอเหมาะกับผู้รับ</font>
            2. <font color='white'>ต้องสามารถส่งลูกได้หลายแบบ</font>
            3. <font color='white'>มองเป้าหมายขณะส่ง เมื่อมีฝ่ายป้องกันควรใช้หางตามองแทนเพราะจะช่วยให้ฝ่ายตรงข้ามตัดบอลได้ยาก</font>
            4. <font color='white'>ก่อนส่งลูกออกไปต้องสังเกตว่าผู้รับพร้อมจะรับหรือไม่</font>
            """, unsafe_allow_html=True)
        url_12 = 'https://www.youtube.com/watch?v=7UT4JTPiw_0&pp=ygUcYm91bmNlIHBhc3MgYmFza2V0YmFsbCBjaGVzdA%3D%3D'
        st.video(url_12)
